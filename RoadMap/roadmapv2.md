...
Các dataset chính như CREMA-D, RAVDESS, TESS và SAVEE có nhãn emotion, không có nhãn stress/non-stress. Nếu nói hệ thống phát hiện stress, sẽ bị hỏi:
...
```text
Training phase
CREMA-D + RAVDESS speech-only + TESS + SAVEE
        ↓
...
**Cập nhật dataset:** phiên bản hiện tại dùng 4 dataset emotion chính gồm **CREMA-D, RAVDESS speech-only, TESS và SAVEE**, giữ 6 nhãn chung `neutral`, `happy`, `sad`, `angry`, `fear`, `disgust`; loại `calm` và `surprise` để đảm bảo label đồng nhất.
...
**CREMA-D + RAVDESS speech-only + TESS + SAVEE** là bộ dữ liệu emotion chính của phiên bản hiện tại.
...
- Bốn dataset này đủ để train baseline classical và tạo mốc so sánh trước khi chuyển sang CNN/SSL trong Colab/Kaggle.

### 4.3. TESS, SAVEE và EmoDB nên dùng như thế nào?

TESS và SAVEE đã được thêm vào pipeline emotion chính để có setup gần hơn với nhiều nghiên cứu dùng RAVDESS + CREMA-D + TESS + SAVEE. EmoDB vẫn nên để làm benchmark phụ nếu còn thời gian.
...
TESS có audio sạch nhưng chỉ có 2 speaker nữ; SAVEE chỉ có 4 speaker nam và quy mô nhỏ. Hai dataset này hữu ích cho so sánh học thuật nhưng cần báo cáo per-dataset metric để tránh hiểu nhầm rằng mô hình generalize tốt trên mọi domain. EmoDB nhỏ và tiếng Đức, nên chỉ để future benchmark.
...
| Unified label | CREMA-D | RAVDESS | TESS | SAVEE | Ghi chú |
|---|---|---|---|---|---|
| neutral | neutral | neutral | neutral | n / neutral | Lớp nền, dễ bị model over-predict |
| happy | happy | happy | happy | h / happy | Dùng để nhận diện giọng sáng/tích cực |
| sad | sad | sad | sad | sa / sad | Có thể liên quan energy thấp |
| angry | anger | angry | angry | a / angry | Có thể energy cao, tone mạnh |
| fear | fear | fearful | fear | f / fear | Có thể liên quan thiếu tự tin nhưng không kết luận stress |
| disgust | disgust | disgust | disgust | d / disgust | Dễ nhầm với angry |
...
- Thư mục TESS.
- Thư mục SAVEE, thường ở dạng `SAVEE/ALL/*.wav`.
- Tùy chọn EmoDB cho benchmark phụ.
...
Từ folder hoặc filename xác định file thuộc CREMA-D, RAVDESS, TESS hay SAVEE.
...
| dataset | Có | CREMA-D, RAVDESS, TESS, SAVEE, EmoDB |
...
- Tải CREMA-D, RAVDESS speech-only, TESS và SAVEE.
- Tạo parser label.
- Map 6 emotion.
- Tạo metadata.
...
- Parse label CREMA-D/RAVDESS/TESS/SAVEE.
- Tạo metadata.
...
- Metadata clean cho CREMA-D + RAVDESS + TESS + SAVEE.
...
- Speech rate.
- Filler words.
- TESS và SAVEE đã dùng trong bản chính; EmoDB để benchmark phụ.
